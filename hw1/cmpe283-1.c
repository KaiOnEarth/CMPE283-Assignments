 ...
 */
struct capability_info exit_controls[14] =
{
 ...
 */
struct capability_info entry_controls[12] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exit_controls, 14, lo, hi);
	
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entry_controls, 12, lo, hi);
}
